#!/usr/bin/env bash
# Smoke test for a built cert-manager acmesolver image. Usage: test.sh <image-ref>
...
echo "checking the acmesolver CLI responds"
...
echo "$out" | grep -qi "acmesolver" || { echo "missing 'acmesolver' in --help"; echo "$out"; exit 1; }

user="$(docker inspect "$IMAGE" --format '{{.Config.User}}')"
[ "$user" = "1001" ] || { echo "expected user 1001, got '$user'"; exit 1; }
echo "smoke test passed (acmesolver CLI responds, nonroot user: $user)"
