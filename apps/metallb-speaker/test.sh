#!/usr/bin/env bash
# Smoke test for a built metallb-speaker image. Usage: test.sh <image-ref> <version>
# The speaker needs a Kubernetes API server to do real work, so outside a cluster it logs
...
echo "$logs" | head -5
echo "$logs" | grep -q "MetalLB speaker starting version ${VERSION}" \
  || { echo "startup line with version ${VERSION} not found"; exit 1; }
echo "$logs" | grep -q '"commit":"[0-9a-f]\{8\}"' \
  || { echo "commit not stamped"; exit 1; }

echo "smoke test passed (nonroot user $user, speaker version ${VERSION})"
