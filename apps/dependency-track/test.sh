...
docker run -d --name "$TAG-pg" --network "$NET" --network-alias pg \
  -e POSTGRES_PASSWORD="$PW" -e POSTGRES_DB=dtrack "$PG" >/dev/null
# (the QuenchWorks postgresql image creates POSTGRES_DB only when it differs from the
# superuser name, so the test keeps the default superuser and names the app database)
# the server does not retry its database at startup, it exits: wait for PostgreSQL
for i in $(seq 1 60); do
  docker exec "$TAG-pg" pg_isready -h 127.0.0.1 -U postgres -d dtrack >/dev/null 2>&1 && break
...
docker run -d --name "$TAG-api" --network "$NET" -p 127.0.0.1:18080:8080 -p 127.0.0.1:19000:9000 \
  ...
  -e DT_DATASOURCE_USERNAME=postgres -e DT_DATASOURCE_PASSWORD="$PW" "$IMAGE" >/dev/null
