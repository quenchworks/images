#!/usr/bin/env bash
# Smoke test for a built cert-manager cainjector image. Usage: test.sh <image-ref>
...
echo "checking the cainjector CLI responds"
...
echo "$out" | grep -qi "cainjector" || { echo "missing 'cainjector' in --help"; echo "$out"; exit 1; }

user="$(docker inspect "$IMAGE" --format '{{.Config.User}}')"
[ "$user" = "1001" ] || { echo "expected user 1001, got '$user'"; exit 1; }
echo "smoke test passed (cainjector CLI responds, nonroot user: $user)"
