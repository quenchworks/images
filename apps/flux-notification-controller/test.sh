#!/usr/bin/env bash
# Smoke test for a built Flux notification-controller image. Usage: test.sh <image-ref> [version]
...
echo "smoke test passed (Flux notification-controller, uid $user, dead-API boot on a read-only root)"
