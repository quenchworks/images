#!/usr/bin/env bash
# Smoke test for a built cert-manager webhook image. Usage: test.sh <image-ref>
...
echo "checking the webhook CLI responds"
...
echo "$out" | grep -qi "webhook" || { echo "missing 'webhook' in --help"; echo "$out"; exit 1; }

user="$(docker inspect "$IMAGE" --format '{{.Config.User}}')"
[ "$user" = "1001" ] || { echo "expected user 1001, got '$user'"; exit 1; }
echo "smoke test passed (webhook CLI responds, nonroot user: $user)"
