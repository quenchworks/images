#!/usr/bin/env bash
# Smoke test for a built Flux source-controller image. Usage: test.sh <image-ref> [version]
...
out="$(timeout 25 docker run --rm --read-only --tmpfs /tmp -e KUBECONFIG=/etc/kubeconfig \
  -v "$WORK/kubeconfig:/etc/kubeconfig:ro" "$IMAGE" --storage-path=/tmp/data 2>&1 || true)"
...
echo "smoke test passed (Flux source-controller, uid $user, dead-API boot on a read-only root)"
