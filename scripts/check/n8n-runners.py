...
# npm `n8n` package's `stable` dist-tag line. NEWEST-ONLY: we ship only the newest stable
# minor line (older lines fail the comprehensive gate on stale Node deps), so build.conf
# VERSIONS is a single entry and report_lines reports just that line. (The bundled component
# versions — @n8n/task-runner, task-runner-launcher, the python runner — are pinned per n8n
# version inside apps/n8n-runners/build.conf.)
